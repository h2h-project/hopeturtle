...
def _show_last_distance(device):
    ...
    # Format: keep it short for 128x64
...
def main():
    if len(sys.argv) < 2:
        print("Usage: oled_status.py [boot-waking|boot-alive|gps-searching|swim|distance]")
        return 0
    ...
    try:
        ...
        # Never crash boot if OLED fails—log and continue
        traceback.print_exc()
    finally:
        _clear(device)
    return 0
